...
public class ReaderImpl extends AbsReader {
    ...
    public void read() throws RuntimeException {
        ...
        try {
            ...
            if (SLog.isDebug()) {
                SLog.i("read head: " + BytesUtils.byteArrayToHexString(headBuf.array()));
            }
            ...
            if (bodyLength > 0) {
                ...
                bodyByteBuffer.order(mOkOptions.getReadByteOrder());
                if (mRemainingBuf != null) {
                    int bodyStartPosition = mRemainingBuf.position();
                    int length = Math.min(mRemainingBuf.remaining(), bodyLength);
                    bodyByteBuffer.put(mRemainingBuf.array(), bodyStartPosition, length);
                    ...
                    if (length == bodyLength) {
                        ...
                        originalData.setBodyBytes(bodyByteBuffer.array());
                ...
                readBodyFromChannel(bodyByteBuffer);
                originalData.setBodyBytes(bodyByteBuffer.array());
    ...
    private void readBodyFromChannel(ByteBuffer byteBuffer) throws IOException {
        while (byteBuffer.hasRemaining()) {
            try {
                ...
                if (len > remaining) {
                    ...
                    mRemainingBuf.put(bufArray, remaining, len - remaining);//接收的缓存
                } else {
        ...
        if (SLog.isDebug()) {
            SLog.i("read total bytes: " + BytesUtils.byteArrayToHexString(byteBuffer.array()));
