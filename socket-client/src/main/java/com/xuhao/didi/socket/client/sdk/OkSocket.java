...
//    private static ManagerHolder holder = ManagerHolder.getInstance();
    ...
    private ManagerHolder holder = null;

    public OkSocket() {
        holder = new ManagerHolder();
    }

    /**
     * 获得一个SocketServer服务器.
     */
    public IRegister<IServerActionListener, IServerManager> server(int serverPort) {
     ...
     */
    public IConnectionManager open(ConnectionInfo connectInfo) {
     ...
     * @return 该参数的连接管理器 {@link IConnectionManager} 连接参数仅作为配置该通道的参配,不影响全局参配
     */
    public IConnectionManager open(String ip, int port) {
     ...
     * @deprecated
     */
    public IConnectionManager open(ConnectionInfo connectInfo, OkSocketOptions okOptions) {
     ...
     * @deprecated
     */
    public IConnectionManager open(String ip, int port, OkSocketOptions okOptions) {
        ConnectionInfo info = new ConnectionInfo(ip, port);
